  ...
  async function handleRequest(request) {
    ...
    if (pathname === "/") {
      ...
      for (let i = 0; i < jsonObject.length; i++) {
        const jsonObject_n = JSON.parse(jsonObject[i]);
        ...
        let proxyFieldValue;
        if (!allProxyType.includes(proxyType)) {
          auth = findFieldValue(jsonObject_n, "auth_str");
          alpn = findFieldValue(jsonObject_n, "alpn");
          proxyFieldValue = findFieldValue(jsonObject_n, "proxy");
        }
        if (proxyFieldValue) {
...
${JSON.stringify(successful_urls)}`,
  ...
  async function read_htmlContent_convert_json() {
    const jsonString_set = /* @__PURE__ */ new Set();
    const yamlString_set = /* @__PURE__ */ new Set();
    const yaml_proxies_set = /* @__PURE__ */ new Set();
    try {
      const results = await fetchMultiplePages(urls, batchSize);
      results.forEach((element) => {
        try {
          let jsonObject_str = JSON.stringify(JSON.parse(element), null, 2);
          let outbounds = findFieldValue(JSON.parse(jsonObject_str), "outbounds") || "";
          if (outbounds.length > 0) {
            outbounds.forEach(function(item) {
              if (typeof item === "object") {
                let node = JSON.stringify(item, null, 2);
                jsonString_set.add(node);
              }
            });
          } else {
            jsonString_set.add(jsonObject_str);
          }
        } catch (error) {
          try {
            const yamlString = js_yaml_default.load(element.replace(/!<str>/g, ""));
            yaml_proxies_set.add(findFieldValue(yamlString, "proxies"));
          } catch (error2) {
          }
        }
      });
    ...
    yaml_proxies_set.forEach(function(item) {
      if (typeof item === "object") {
        item.forEach(function(subItem) {
          let node = JSON.stringify(subItem, null, 2);
          yamlString_set.add(node);
        });
      }
    });
    const uniqueYamlResults = Array.from(yamlString_set);
    const uniqueJsonResults = Array.from(jsonString_set);
    let mergedArray = uniqueYamlResults.concat(uniqueJsonResults);
    return mergedArray;
  }
  async function fetchMultiplePages(urls2, batchSize2, timeout = 8e3) {
    const results = [];
    for (let i = 0; i < urls2.length; i += batchSize2) {
      const batchUrls = urls2.slice(i, i + batchSize2);
  ...
  function parse_hysteria(jsonObject_n) {
    ...
    if (port) {
      ...
      hy1 = `hysteria://${server}?${encodedParams}#[hysteria]_${server}`;
    }
    return hy1;
  }
  ...
  function parse_vless(jsonObject_n) {
    ...
    let encryption = findFieldValue(jsonObject_n, "encryption") || "none";
  ...
  function parse_trojan(jsonObject_n) {
    ...
    let sni = findFieldValue(jsonObject_n, "sni") || "";
    let fp = findFieldValue(jsonObject_n, "client-fingerprint") || findFieldValue(jsonObject_n, "fingerprint") || "";
    let alpn = findFieldValue(jsonObject_n, "alpn") || "";
    let tls_security = "";
    if (sni) {
      tls_security = "tls";
    }
