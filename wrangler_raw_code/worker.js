...
function parse_vmess1(outbounds_n) {
  let address = findFieldValue(outbounds_n, "address") || findFieldValue(outbounds_n, 'server');
  if (address === "127.0.0.1") {
    return "";
  }
  ...
  let network = findFieldValue(outbounds_n, 'network');
  // 伪装类型(type)
  let type_encryption = findFieldValue(outbounds_n, 'encryption') || "auto";
  ...
  // 伪装域名(host)
  let host = findFieldValue(outbounds_n, 'Host') || findFieldValue(outbounds_n, 'host') || "";
  let serverName = findFieldValue(outbounds_n, 'sni') || findFieldValue(outbounds_n, 'serverName') || findFieldValue(outbounds_n, 'servername') || "";
  if (host === "" && serverName === "") {
    host = address
  } else if (host === "" && serverName !== "") {
    host = serverName
...
async function processUrls(targetUrls) {
  ...
  // 并发执行的最大任务数量
  const maxConcurrency = 3; // 你可以根据需求调整这个数值
...
// 定义一个包含多个URL的数组(顺序随意，是json、yaml数据都可以，前提是抓取网页内容不会抛出“internal error”错误)
const targetUrls = [
  // hysteria
